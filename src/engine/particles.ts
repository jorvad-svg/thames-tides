...
export function updateAndDrawParticles(
  ...
  for (let i = 0; i < particles.length; i++) {
    ...
    drawSmooth(0, len);
    ctx.strokeStyle = levelToParticleColor(currentLevel, baseAlpha * 0.25 + pointerBoost * 0.1, themeBlend);
    ...
    drawSmooth(s2, len);
    ctx.strokeStyle = levelToParticleColor(currentLevel, baseAlpha * 0.5 + pointerBoost * 0.15, themeBlend);
    ...
    drawSmooth(s3, len);
    ctx.strokeStyle = levelToParticleColor(currentLevel, Math.min(baseAlpha * 0.9 + pointerBoost * 0.3, 1.0), themeBlend);
